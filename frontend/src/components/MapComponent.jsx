...
const MapComponent = ({ setRestaurants, setHotels, setAttractions }) => {
  ...
  // using a high-order function in the ExplorePageUtils file to avoid repeating
  const getNearbyRestaurants = createNearbySearch(
    "restaurant",
    restaurantCallback,
    coords,
    map
  );
  const getNearbyAttraction = createNearbySearch(
    "tourist_attraction",
    attractionCallback,
    coords,
    map
  );
  const getNearbyHotels = createNearbySearch(
    "lodging",
    hotelCallback,
    coords,
    map
